...
const Section = ({ h3, text, hasBtn = true, btnTxt, imgSrc, imgSize = "70%", backgroundColor, headingColor, textColor, btnBgColor, btnColor }) => {
  ...
  return (
    <section className='section' style={{ backgroundColor: backgroundColor }}>
      <div>
        <motion.h3 initial={{ y: "100%", opacity: 0 }} whileInView={{ y: "0", opacity: 1 }} style={{ color: headingColor }} data-cursorpointer={true} {...headingOptions} >{h3}</motion.h3>
        <motion.p style={{ color: textColor }} data-cursorpointer={true} {...textOptions} >{text}</motion.p>

        {
          hasBtn && <motion.button style={{ color: btnColor, backgroundColor: btnBgColor, borderRadius: "7px" }} data-cursorpointer={true} {...buttonOptions} >{btnTxt}</motion.button>
        }
